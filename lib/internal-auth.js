...
export function verifyInternalAuth(req, rawBody) {
  const configuredSecret = process.env.N8N_WEBHOOK_SECRET || 'inboxiq_production_orchestration_secret_key_2026';
  ...
  if (secretHeader) {
    for (const validSecret of VALID_INTERNAL_SECRETS) {
      ...
      if (secretHeader.length === validSecret.length) {
        try {
          if (crypto.timingSafeEqual(Buffer.from(secretHeader, 'utf8'), Buffer.from(validSecret, 'utf8'))) {
            return {
              correlationId,
              timestamp: Date.now(),
            };
