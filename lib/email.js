 ...
 */
export async function sendTrialExpiredRenewalReminderEmail({ email, name, planName, reportTime, timezone }) {
  const fromEmail = process.env.EMAIL_FROM || process.env.EMAIL_USER || 'no-reply@inboxiq.online';
  const transporter = getEmailTransporter();
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff;">
  ...
  if (!transporter) {
    console.log('[sendTrialExpiredRenewalReminderEmail] Simulated email dispatch to:', email);
    return { success: true, simulated: true };
  }
 ...
 * Triggered by the hourly cron dispatcher at their configured report_time.
 */
export async function sendSubscriptionExpiredRenewalReminderEmail({ email, name, planName, reportTime }) {
  const fromEmail = process.env.EMAIL_FROM || process.env.EMAIL_USER || 'no-reply@inboxiq.online';
  const transporter = getEmailTransporter();
  const renewUrl = 'https://www.inboxiq.online/billing';
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff;">
  ...
  if (!transporter) {
    console.log('[sendSubscriptionExpiredRenewalReminderEmail] Simulated email dispatch to:', email);
    return { success: true, simulated: true };
  }
