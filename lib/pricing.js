...
/**
 * Server-side trial configuration — single source of truth.
 * Duration, limits, and label are defined here and used by the trial claim API.
 */
...
/**
 * Razorpay plans are created in the Razorpay dashboard and are immutable by
 * amount/currency. Keep one monthly plan ID for each sellable tier/currency.
 */
export function getRecurringRazorpayPlanId(profession = 'professor_teacher', currency = 'INR') {
  const tier = profession === 'student' ? 'STUDENT' : profession === 'others' ? 'PROFESSIONAL' : 'FACULTY';
  return process.env[`RAZORPAY_PLAN_ID_${tier}_${currency}`] || null;
}
